...
let CidadesService = class CidadesService {
    ...
    async findOne(codigobge) {
        const cidade = await this.cidadeRepository.findOne({
            where: { codigobge },
            relations: ['estadoRel']
        });
        if (!cidade) {
            throw new common_1.NotFoundException(`Cidade com o código IBGE #${codigobge} não encontrada.`);
        }
        return cidade;
    }
    async update(codigobge, updateCidadeDto) {
