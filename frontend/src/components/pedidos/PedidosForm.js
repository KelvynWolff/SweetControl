...
const PedidosForm = () => {
  ...
  const handleAddItem = () => {
    ...
    const jaExiste = itensPedido.find(
      (i) => i.idProduto === parseInt(currentItem.idProduto),
    );
    if (jaExiste) {
      alert(
        'Este produto já está na lista. Remova-o se quiser adicionar novamente.',
      );
      return;
    }
    ...
    const precoFinal = Math.max(
      0,
      Number(currentItem.precoUnitario) - Number(currentItem.desconto),
    );
          ...
          >
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setCurrentItem({ ...currentItem, desconto: e.target.value })
                }
              />
            </div>
              ...
              <tbody>
                {itensPedido.map((item, index) => (
                  <tr key={index}>
                    <td>
                      {item.nomeProduto}
                      {item.mensagemPromocao && (
                        <div style={{ fontSize: '0.75em', color: 'green' }}>
                          {item.mensagemPromocao}
                        </div>
                      )}
                    </td>
